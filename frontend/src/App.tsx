            ...
            <div className="flex space-x-2">
              {(['7d', '14d', '30d'] as DateRange[]).map((range) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm font-medium ${dateRange === range ? 'bg-wasabi-600' : 'hover:bg-wasabi-400'
                    }`}
                >
                  {range}
                </button>
      ...
      <main className="container flex flex-col gap-6 mx-auto px-4 py-6">
        <div className="bg-white shadow rounded-lg p-6">
          <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <WarehouseFilter value={warehouseFilter} onChange={handleWarehouseChange} />

          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <KPICard title="Total Stock" value={totalStock.toString()} trend="up" />
          <KPICard title="Total Demand" value={totalDemand.toString()} trend="down" />
          <KPICard title="Fill Rate" value={`${fillRate}%`} trend={fillRate > 80 ? 'up' : 'down'} />
        </div>
